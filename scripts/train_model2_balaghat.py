...
def fit_annual_trend_qualitative():
    print("\n--- Qualitative Benchmarking: 5-Year Annual Trend ---")
    
    # Real 5-year data (in million tonnes, converting to actual tonnes)
    years = [2015, 2016, 2017, 2018, 2019]
    rom_actual = [315000, 297000, 326000, 341000, 374000]
    ...
    print("\nPredicting year 2020 for trend plausibility (qualitative check):")
    
    # 1. Holt-Winters
    try:
        hw = ExponentialSmoothing(rom_actual, trend='add', seasonal=None, initialization_method='estimated').fit()
        hw_pred = hw.forecast(1)[0]
        print(f"Holt-Winters:      {hw_pred:,.0f} t (trend continues upward)")
    except Exception as e:
        print(f"Holt-Winters failed: {e}")
        
    # 2. SARIMAX (simple ARIMA 1,1,0 for short series)
    try:
        sar = SARIMAX(rom_actual, order=(1,1,0)).fit(disp=False)
        sar_pred = sar.forecast(1)[0]
        print(f"SARIMAX (1,1,0):   {sar_pred:,.0f} t (plausible extrapolation)")
    except Exception as e:
    ...
    # 4. XGBoost & CatBoost (treat year as feature)
    X = np.array(years).reshape(-1, 1)
    y = np.array(rom_actual)
    X_future = np.array([[2020]])
    
    xgb = XGBRegressor(n_estimators=10, max_depth=2, random_state=42)
    xgb.fit(X, y)
    print(f"XGBoost:           {xgb.predict(X_future)[0]:,.0f} t (step-function limits extrapolation)")
    
    cat = CatBoostRegressor(iterations=10, depth=2, random_state=42, verbose=0)
    cat.fit(X, y)
    print(f"CatBoost:          {cat.predict(X_future)[0]:,.0f} t (step-function limits extrapolation)")
    
    print("\nConclusion: Tree-based models (XGB/CatBoost) fail to extrapolate smoothly on a pure 1D time index.")
    print("Classical TS models (HW, SARIMAX) produce plausible smooth upward trends.")

def generate_daily_synthetic():
    print("\n--- Quantitative Daily Calibration (Synthetic wrapping real annuals) ---")
    
    years = {2015: 315000, 2016: 297000, 2017: 326000, 2018: 341000, 2019: 374000}
    ...
    for y, target in years.items():
        ...
        # Beneficiation step (70% recovery)
        df_year['processed_tonnes'] = df_year['rom_tonnes'] * 0.70
        
        # Grade tiers (no 35-46% tier)
        def price_logic(processed):
            if processed == 0: return 0
            tier_weights = [0.2, 0.5, 0.3] # below 25, 25-35, 46+
            prices = [3000, 5000, 15000] # arbitrary mock prices
            val = (processed * tier_weights[0] * prices[0]) + \
                  (processed * tier_weights[1] * prices[1]) + \
                  (processed * tier_weights[2] * prices[2])
            return val
    ...
    print(f"Generated {len(df)} daily points over 5 years.")
...
def train_daily_model(df):
    ...
    if cat_rmse < xgb_rmse:
        ...
        out_path = os.path.join(models_dir, 'model2_balaghat_ug.cbm')
        cat.save_model(out_path)
    else:
        print("Saving XGBoost model...")
        out_path = os.path.join(models_dir, 'model2_balaghat_ug.json')
